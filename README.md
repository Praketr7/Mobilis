...
**Gridlock turns 298,450 historical violation records into specific, time-aware, actionable deployment decisions — delivered via a command dashboard and a Telegram bot that any BTP officer can use without training.**

---

## What Gridlock Does

Gridlock has three layers:
...
### 3. Telegram Bot
Field-facing interface. Officers query in natural language, get back specific action cards.
...
│              │  /hotspots       │                       │
│  Preprocessed│  /action-cards   │  Telegram Bot         │
│  JSON cache  │  /daily-brief    │  (field officers)     │
│              │  /query          │                       │
│  DBSCAN      │                  │  Claude/Groq API      │
...
### Screen 5: Daily Intelligence Briefing
- Auto-generated morning report (mimics what a station in-charge would read at 7 AM)
- Structured as: Top 5 hotspots for today (day-of-week matched) → Recommended unit deployment → Vehicle type advisories → Junctions to watch in evening peak
- Exportable as PDF
- Generated by Claude/Groq API with junction data as context
...
The bot is designed for field officers and station in-charges who need answers fast without opening a dashboard.

### Commands

| Command | Description |
|---|---|
| `/start` | Onboarding message + quick guide |
| `/hotspots` | Top 5 active hotspots right now (time-aware) |
| `/junction <name or BTP code>` | Full intelligence card for a specific junction |
| `/deploy <station name> <time>` | Deployment recommendation for a police station at a given time |
| `/brief` | Today's daily intelligence briefing for the querying officer's station |
| `/status` | City-wide congestion status summary |

### Natural Language Query (AI mode)
...
```
🌅 Gridlock Morning Brief — Wednesday, [Date]
...
| AI decision layer | Claude/Groq API |
...
| Gridlock Output | BTP Decision Enabled |
|---|---|
...
gridlock/
├── backend/
│   ├── preprocess.py          # One-time data processing, outputs JSON cache
│   ├── clustering.py          # DBSCAN hotspot detection
│   ├── scoring.py             # Congestion Impact Score computation
│   ├── temporal.py            # Hour/day pattern extraction
│   ├── main.py                # FastAPI app + endpoints
│   └── ai_layer.py            # Claude API integration, prompt templates
│
├── bot/
│   ├── bot.py                 # Telegram bot entry point
│   ├── handlers.py            # Command + NL query handlers
│   └── daily_brief.py         # Scheduled 7 AM report generator
│
├── frontend/
│   ├── src/
│   │   ├── pages/
│   │   │   ├── Overview.jsx       # Command overview + CIS leaderboard
│   │   │   ├── HotspotMap.jsx     # Leaflet map + heatmap
│   │   │   ├── JunctionPanel.jsx  # Per-junction intelligence
│   │   │   ├── Temporal.jsx       # Time analysis + deployment planner
│   │   │   └── DailyBrief.jsx     # Morning briefing screen
│   │   └── components/
│   │       ├── ActionCard.jsx
│   │       ├── CISBadge.jsx
│   │       └── HourHeatmap.jsx
│
├── data/
│   ├── violations_raw.csv         # Original dataset (not committed)
│   └── processed/
│       ├── hotspots.json
